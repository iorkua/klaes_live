...
window.openQuickBatchModal = function(selectedInstruments) {
  ...
  // Fetch next serial number with fresh data
  // Prefer centralized fetch which updates both batch and quick-batch displays
  if (typeof fetchNextSerialNumber === 'function') {
    console.log('Fetching fresh serial number for quick batch modal');
    fetchNextSerialNumber()
      .then(() => {
        console.log('Serial number fetched successfully for quick batch');
        // refresh quick-batch table after serial fetched
        populateQuickBatchTable();
      })
      .catch(err => {
        console.error('Error fetching serial via centralized fetch:', err);
        // fallback to quick-batch specific fetch
        fetchNextSerialNumberForQuickBatch();
      });
  } else {
    console.log('Using fallback quick batch serial fetch');
    fetchNextSerialNumberForQuickBatch();
  }
};
...
function fetchNextSerialNumberForQuickBatch() {
  ...
  return fetch(`${base}/instrument_registration/get-next-serial`, {
    method: 'GET',
    headers: headers,
