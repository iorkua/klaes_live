                ...
                <div class="px-6 py-6 bg-gray-50 border-b border-gray-200">
                    <div class="max-w-md mx-auto">
                        <label for="filenoSelect" class="block text-sm font-medium text-gray-700 mb-3 text-center">
                            ...
                            @if(request()->get('url') === 'physical_planning')
    ...
    <script>
        ...
        function hideBettermentBillActions() {
            if (!window.isPhysicalPlanMode) {
                // Hide only the "GENERATE BETTERMENT BILL" sub-tab when URL does NOT have ?url=physical_planning
                $('.betterment-tab-button[data-tab="generate"]').hide();
                $('#betterment-generate-tab').hide();
                
                // Hide calculate and generate buttons when URL does NOT have ?url=physical_planning
                $('#calculate-betterment-btn, #generate-betterment-btn').hide();
                
                // Hide print bill buttons that call printBettermentBill
                $('button[onclick*="printBettermentBill"]').hide();
                
                // If the generate sub-tab is currently active, switch to receipt sub-tab
                if ($('.betterment-tab-button[data-tab="generate"]').hasClass('active')) {
                    $('.betterment-tab-button').removeClass('active');
                    $('.betterment-tab-button[data-tab="receipt"]').addClass('active');
                    $('.betterment-tab-content').removeClass('active');
                    $('#betterment-receipt-tab').addClass('active');
                }
            } else {
                // Show the "GENERATE BETTERMENT BILL" sub-tab when URL has ?url=physical_planning
                $('.betterment-tab-button[data-tab="generate"]').show();
                $('#betterment-generate-tab').show();
            }
        }
