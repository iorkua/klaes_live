    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function setupPropertyFormDialog() {
            ...
            function showDialog(mode = 'property') {
                ...
                setTimeout(() => {
                    const formContainer = dialog.querySelector('[x-data]');
                    if (formContainer) {
                        // Try multiple methods to set the Alpine.js data
                        if (formContainer._x_dataStack && formContainer._x_dataStack[0]) {
                            formContainer._x_dataStack[0].formMode = mode;
                            console.log('Set form mode via _x_dataStack to:', mode);
                        } else if (window.Alpine && formContainer._x_dataStack) {
                            // Alternative method using Alpine's data access
                            const alpineData = window.Alpine.data(formContainer);
                            if (alpineData) {
                                alpineData.formMode = mode;
                                console.log('Set form mode via Alpine.data to:', mode);
                            }
                        }
                        
                        // Force Alpine to re-evaluate
                        if (formContainer._x_dataStack && formContainer._x_dataStack[0]) {
                            formContainer._x_dataStack[0].$nextTick = formContainer._x_dataStack[0].$nextTick || function() {};
                        }
                        
                        // Also try setting via dispatching custom event
                        formContainer.dispatchEvent(new CustomEvent('form-mode-changed', { 
                            detail: { mode: mode } 
                        }));
                    }
                    ...
                    if (titleElement) {
                        ...
                        console.log('Updated title directly to:', titleElement.textContent);
                    }
                }, 100);
