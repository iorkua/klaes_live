                ...
                <table class="min-w-full divide-y divide-gray-200" id="mlsTable">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick="sortTable('CreationMode')">
                                Source <i data-lucide="chevrons-up-down" class="inline h-3 w-3"></i>
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onclick="sortTable('DateCreated')">
                                Date Commissioned <i data-lucide="chevrons-up-down" class="inline h-3 w-3"></i>
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Commissioned By
                            </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                        @if(isset($mlsFileNumbers) && count($mlsFileNumbers) > 0)
                            ...
                            <tr class="hover:bg-gray-50 transition-colors" data-file-id="{{ $mls->id }}">
...
<script>
    ...
    function calculateStatistics() {
        ...
        rows.forEach(row => {
            // Skip rows that don't have enough cells (like empty state)
    ...
    function applyFilters() {
        ...
        rows.forEach(row => {
            // Skip rows that don't have enough cells (like empty state)
            if (row.cells.length < 6) {
                row.style.display = search || year || status ? 'none' : '';
                return;
            }
            ...
            // Year filter
            if (year && row.cells[3] && !row.cells[3].textContent.includes(year)) {
                show = false;
            }
            
            // Status filter
            if (status && row.cells[2] && !row.cells[2].textContent.toLowerCase().includes(status)) {
                show = false;
            }
    ...
    function editFile(fileNumber) {
        ...
        // Get file ID from table row
        const row = document.querySelector(`tr td:first-child span:contains('${fileNumber}')`);
        let fileId = null;
        
        // Search through all rows to find the matching file number
        const rows = document.querySelectorAll('#tableBody tr');
        rows.forEach(r => {
            const fileNumCell = r.querySelector('td:first-child span');
            if (fileNumCell && fileNumCell.textContent.trim() === fileNumber) {
                // Get the file ID from data attribute or fetch from server
                fileId = r.dataset.fileId;
            }
        });

        // Fetch file details
        fetch(`/mls-fileno/${encodeURIComponent(fileNumber)}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: data.message || 'Failed to fetch file details',
            ...
            .catch(error => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Failed to fetch file details',
                    confirmButtonColor: '#ef4444'
                });
            });
    }
    ...
    function showEditModal(fileData) {
        Swal.fire({
            title: '<i data-lucide="edit" class="inline-block h-6 w-6 mr-2"></i>Edit MLS File Number',
            html: `
                <div class="text-left space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">File Number</label>
                        <input type="text" id="edit_fileNumber" value="${fileData.mlsfNo || ''}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100" readonly>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">File Name / Application Name</label>
                        <input type="text" id="edit_fileName" value="${fileData.FileName || ''}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-green-500"
                               placeholder="Enter file name">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Location</label>
                        <input type="text" id="edit_location" value="${fileData.location || ''}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-green-500"
                               placeholder="Enter location">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Commissioning Date</label>
                        <input type="date" id="edit_commissioningDate" 
                               value="${fileData.commissioning_date ? fileData.commissioning_date.split(' ')[0] : ''}" 
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    </div>
                    <div class="bg-gray-50 p-3 rounded-md text-xs text-gray-600">
                        <p><strong>Source:</strong> ${fileData.SOURCE || 'N/A'}</p>
                        <p><strong>Created:</strong> ${fileData.created_at ? new Date(fileData.created_at).toLocaleString() : 'N/A'}</p>
                        <p><strong>Created By:</strong> ${fileData.created_by || 'System'}</p>
                    </div>
                </div>
            `,
            width: '600px',
            showCancelButton: true,
            confirmButtonText: '<i data-lucide="save" class="inline h-4 w-4 mr-1"></i>Save Changes',
            cancelButtonText: 'Cancel',
            confirmButtonColor: '#16a34a',
            cancelButtonColor: '#6b7280',
            customClass: {
                popup: 'mls-edit-modal'
            },
            didOpen: () => {
                lucide.createIcons();
            },
            preConfirm: () => {
                const fileName = document.getElementById('edit_fileName').value.trim();
                ...
                return {
                    ...
                    commissioning_date: document.getElementById('edit_commissioningDate').value
                };
            }
        }).then((result) => {
            if (result.isConfirmed) {
                updateFileNumber(result.value);
            }
        });
    }

    function updateFileNumber(formData) {
        // Show saving state
        Swal.fire({
            title: 'Saving...',
            text: 'Updating file number',
            allowOutsideClick: false,
            didOpen: () => {
                Swal.showLoading();
            }
        ...
        .then(data => {
            if (data.success) {
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: data.message || 'File number updated successfully',
                    confirmButtonColor: '#16a34a',
                    timer: 2000
                }).then(() => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: data.message || 'Failed to update file number',
                    confirmButtonColor: '#ef4444'
                });
            }
        })
        .catch(error => {
            console.error('Error updating file number:', error);
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to update file number',
                confirmButtonColor: '#ef4444'
            });
        });
    }
