...
<body class="py-2">
    ...
    $sharedUtilitiesFromDb = collect($utilities)
        ->map(function ($value) {
            // Handle both objects with utility_type property and plain strings
            if (is_object($value) && isset($value->utility_type)) {
                return $value->utility_type;
        ...
        ->unique()
        ->values()
        ->toArray();        $sharedAreasCombinedRaw = array_values(array_filter(array_map(function ($value) {
            return is_string($value) ? trim($value) : $value;
        }, array_merge(
        ...
        $unitMeasurementsCollection = collect($unitMeasurements)->map(function ($item) {
            ...
            // For sub-applications, always show the unit even if measurement is empty
            // Only filter out if unit_no is completely missing
            if ($unitNo === '' || is_null($unitNo)) {
                return null;
            }
            ...
            return (object) [
                ...
                'unit_size' => $measurementString, // For backward compatibility
        ...
        <div class="mb-4 text-xs">
            ...
            <div class="flex items-center space-x-4 mb-2">
                <span class="flex items-center">
                    <span class="inline-block w-3 h-3 mr-2 rounded-full border {{ $report && $report->has_additional_observations ? 'bg-blue-500 border-blue-600' : 'border-gray-400' }}"></span>
                    Yes
                </span>
                <span class="flex items-center">
                    <span class="inline-block w-3 h-3 mr-2 rounded-full border {{ !$report || !$report->has_additional_observations ? 'bg-blue-500 border-blue-600' : 'border-gray-400' }}"></span>
            ...
            <div class="border border-dotted border-gray-400 min-h-[70px] p-2 rounded">
                {!! nl2br(e($report && $report->additional_observations ? $report->additional_observations : 'No additional observations recorded.')) !!}
            </div>
        </div>
                ...
                <tbody>
                    @forelse($unitMeasurementsCollection as $index => $unit)
                        <tr>
                            ...
                            <td>
                                @if(!empty($unit->unit_size) || !empty($unit->measurement))
                                    {{ $unit->unit_size ?? $unit->measurement ?? 'N/A' }}
                                    @if($unit->unit_size || $unit->measurement) m²@endif
                                @else
                                    N/A
                                @endif
                            </td>
